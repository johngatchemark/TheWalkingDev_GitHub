...
function App() {
  ...
  const [showMmdaDashboard, setShowMmdaDashboard] = useState<boolean>(false);
  ...
  if (showMmdaDashboard) {
    return <MmdaDashboard onClose={() => setShowMmdaDashboard(false)} />;
  }
  ...
  return (
    <div className={`app-container ${isNight ? 'night-theme' : ''} ${isNavigating ? 'is-navigating' : ''}`}>
      ...
      {/* MMDA Portal Button */}
      <button 
        className="mmda-portal-btn glass-panel" 
        onClick={() => setShowMmdaDashboard(true)}
      >
        <ShieldAlert size={16} />
        <span className="hidden-mobile">MMDA Portal</span>
      </button>
